...
* If the file is present in the @dropsync@ folder (e.g. @dropsync/.bashrc@) and a file (_note: not a symbolic link_) exists in the original location (e.g. @/home/<username>/.bashrc@), then the conflict is resolved by keeping the latest modified file and applying one of the two preceding policies accordingly.
